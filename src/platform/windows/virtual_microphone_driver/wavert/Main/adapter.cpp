...
/** PortCls dispatch callbacks preserved before installing control routing. */
static PDRIVER_DISPATCH g_PortClsDispatch[IRP_MJ_MAXIMUM_FUNCTION + 1] = {};

/**
 * Route every supported request to the secured control device or the original PortCls path.
 *
 * @param DeviceObject Dispatch target.
 * @param Irp Current request.
 * @return Completion status from the selected device path.
 */
_Dispatch_type_(IRP_MJ_CREATE)
_Dispatch_type_(IRP_MJ_CREATE_NAMED_PIPE)
DRIVER_DISPATCH LumenVmicDispatch;
NTSTATUS LumenVmicDispatch(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    PIO_STACK_LOCATION stack = IoGetCurrentIrpStackLocation(Irp);

    if (LumenVirtualMicrophoneControlOwnsDevice(DeviceObject))
    {
        switch (stack->MajorFunction)
        {
        case IRP_MJ_CREATE:
        case IRP_MJ_CLOSE:
            return LumenVirtualMicrophoneControlCreateClose(DeviceObject, Irp);
        case IRP_MJ_CLEANUP:
            return LumenVirtualMicrophoneControlCleanup(DeviceObject, Irp);
        case IRP_MJ_DEVICE_CONTROL:
            return LumenVirtualMicrophoneControlDeviceControl(DeviceObject, Irp);
        default:
            return LumenVirtualMicrophoneControlUnsupported(DeviceObject, Irp);
        }
    }

    if (stack->MajorFunction > IRP_MJ_MAXIMUM_FUNCTION ||
        g_PortClsDispatch[stack->MajorFunction] == NULL)
    {
        return LumenVirtualMicrophoneControlUnsupported(DeviceObject, Irp);
    }
    return g_PortClsDispatch[stack->MajorFunction](DeviceObject, Irp);
}
...
--*/
{
    ...
    LumenVirtualMicrophoneControlShutdown();
...
--*/
    ...
    ntStatus = LumenVirtualMicrophoneControlInitialize(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("Lumen microphone control initialization failed, 0x%x", ntStatus)),
        Done);

    for (UCHAR major = 0; major <= IRP_MJ_MAXIMUM_FUNCTION; ++major)
    {
        g_PortClsDispatch[major] = DriverObject->MajorFunction[major];
        if (major != IRP_MJ_PNP)
        {
            DriverObject->MajorFunction[major] = LumenVmicDispatch;
        }
    }
