...
#define LUMEN_PCM_RING_H
  ...
  #include <ntddk.h>
typedef UINT64 lumen_pcm_uint64_t;
typedef INT16 lumen_pcm_int16_t;
typedef SIZE_T lumen_pcm_size_t;
#else
  #include <stddef.h>
  #include <stdint.h>
typedef uint64_t lumen_pcm_uint64_t;
typedef int16_t lumen_pcm_int16_t;
typedef size_t lumen_pcm_size_t;
#endif
...
extern "C" {
  ...
  typedef struct LUMEN_PCM_RING_COUNTERS {
    lumen_pcm_uint64_t submitted_frames;  ///< Frames retained from successful submissions.
    lumen_pcm_uint64_t consumed_frames;  ///< Queued frames returned to a consumer.
    lumen_pcm_uint64_t silence_frames;  ///< Zero frames synthesized on underflow.
    lumen_pcm_uint64_t dropped_frames;  ///< Oldest queued or oversized-input frames discarded.
    lumen_pcm_uint64_t reset_count;  ///< Number of FIFO resets.
  ...
  typedef struct LUMEN_PCM_RING {
    lumen_pcm_int16_t *storage;  ///< Caller-owned sample storage.
    lumen_pcm_size_t capacity_frames;  ///< Number of frames storage can retain.
    lumen_pcm_size_t read_index;  ///< Index of the oldest queued frame.
    lumen_pcm_size_t queued_frames;  ///< Number of queued frames.
