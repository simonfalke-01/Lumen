...
namespace video {
  ...
  int avcodec_software_encode_device_t::convert(platf::img_t &img) {
    ...
    // PipeWire can deliver NV12 with a one-byte pixel pitch, while the other
    // capture paths provide packed BGR0. Fall back to the row pitch for
    // backends that do not report pixel_pitch.
  ...
  void avcodec_software_encode_device_t::prefill() {
    auto active_frame = sw_frame ? sw_frame.get() : frame;
  ...
  int avcodec_software_encode_device_t::init(int in_width, int in_height, AVFrame *in_frame, AVPixelFormat format, bool hardware) {
    ...
    const auto scalar = std::fminf(static_cast<float>(out_width) / in_width, static_cast<float>(out_height) / in_height);
